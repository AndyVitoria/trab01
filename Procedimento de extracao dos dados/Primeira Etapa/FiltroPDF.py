...
CONTROL = [True, True, True, True]  # Variavel de controle de criação de threads


# ========================{Threads}======================== #
def PreparaConversao(Pilha):
    i = 0
    while i < 4 and CONTROL[i] == False:
        i += 1

    CONTROL[i] = False
    Conversor(Pilha[0], i).start()
    del (Pilha[-1])
    return

# Chamada do tabula com o arquivo e o diretorio para conversao
def converteCSV(Dir):
...
def listaDir():
    # Listagem dos diretorios e arquivos dentro da pasta PDF
...
# Funcao para abertura do PDF usando PyPDF2
def abrirPDF(Dir):
    arq = open(Dir, 'rb')
...
def FiltraPDF(Dir):
    ...
    # Verifica se o boletim e valido e possui os codigos dos crimes desejados
...
def main():
    ...
    for i in range(0, len(diretorios), 1):
        arquivos = abrir('PDF/' + diretorios[i] + '/' + 'lote.txt')[:-1]  # Lista com arquivos do diretorio na posicao i
